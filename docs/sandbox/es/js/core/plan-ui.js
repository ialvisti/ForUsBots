// docs/sandbox/es/js/core/plan-ui.js
// UI builder + JSON body para /forusbot/scrape-plan (ESPAÑOL)
import { $ } from "./utils.js";

/** ====== Módulos de Plan (fuente de verdad para sandbox) ====== */
const PLAN_MODULES = [
  {
    key: "basic_info",
    label: "basic_info",
    description: "ID del plan, nombre de empresa, EIN, estado, fechas (16 campos)",
    fields: {
      mode: "static",
      list: [
        "plan_id",
  ...
  },
  {
    key: "plan_design",
    label: "plan_design",
    description: "Record keeper, elegibilidad, contribuciones, inscripción (26 campos)",
    fields: {
      mode: "static",
      list: [
        "record_keeper_id",
  ...
  },
  {
    key: "onboarding",
    label: "onboarding",
    description: "Primera fecha de diferimiento, fechas de blackout, método de inscripción (6 campos)",
    fields: {
      mode: "static",
      list: [
        "first_deferral_date",
  ...
  },
  {
    ...
    description: "Texto DAVE, logo, participantes en español, e-statement (6 campos)",
    fields: {
      mode: "static",
      list: [
        "dave_text",
  ...
  },
  {
    ...
    description: "Modo de carga RK, año del plan, elegibilidad ER match (10 campos)",
    fields: {
      mode: "static",
      list: [
        "rk_upload_mode",
  ...
  },
  {
    key: "feature_flags",
    label: "feature_flags",
    description: "Payroll X-Ray, problema de nómina, carga simple (3 campos)",
    fields: {
      mode: "static",
      list: [
        "payroll_xray",
...
function makeChkSelect({ key, options = [] }) {
  ...
  btn.textContent = "Seleccionar campos";
  ...
  const updateBtnText = () => {
    ...
    btn.textContent = n
      ? `${n} campo${n > 1 ? "s" : ""} seleccionado${n > 1 ? "s" : ""}`
      : "Seleccionar campos";
  };
...
function renderFieldsForModule(row, key) {
  ...
  if (spec.mode === "none") {
    fieldsBox.innerHTML = `<div class="help">Este módulo no tiene campos seleccionables. El extractor devolverá su payload predeterminado.</div>`;
    return;
  }
...
function buildRow({ key, removable }) {
  ...
  fieldLeft.innerHTML = `<label>módulo</label><select class="plan-module-key"></select>`;
