// docs/sandbox/sandbox.js (ES — solo traducción de textos)
(function () {
  ...
  try {
    ...
    baseUrl.title = "Bloqueado al origen de esta página";
  } catch {}
  ...
  function setFile(f) {
    ...
    if (!(f.type === "application/pdf" || /\.pdf$/i.test(f.name))) {
      runResult.textContent = "Error: arrastra/selecciona un PDF válido.";
      return;
    }
  ...
  function buildSnippets(metaStr) {
    ...
    const tokLiteral = "YOUR_TOKEN_HERE"; // <- nunca mostrar el token real
...
print(res.status_code, res.text)`;
  ...
  function validateBasicsForRun() {
    ...
    if (ep.needs.token) {
      if (!token.value)
        throw new Error("x-auth-token es requerido para este endpoint.");
    ...
    if (ep.needs.xfilename) {
      ...
      if (!/\.pdf$/i.test(xf))
        throw new Error("x-filename debe terminar en '.pdf'.");
    ...
    if (ep.group === "upload") {
      ...
      if (ep.needs.pdf && !file) throw new Error("Selecciona un PDF para probar.");
      ...
      if (
        ...
        throw new Error(
          "Status 'Document Missing' no es válido con archivo adjunto (422). Usa 'Audit Ready'."
        );
      }
    }
  ...
  dryBtn.addEventListener("click", async (e) => {
    ...
    if (endpointSel.value !== "sandbox-upload") {
      runResult.textContent = "Dry-Run aplica al endpoint sandbox-upload.";
      return;
    }
    try {
