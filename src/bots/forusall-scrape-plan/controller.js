...
/** Normaliza modules: acepta ["basic_info"] o [{key:"basic_info", fields:[...]}] */
...
module.exports = async function controller(req, res) {
  try {
    ...
    const planId = body.planId ?? body.planID ?? body.id;
    if (
      planId === undefined ||
      planId === null ||
      String(planId).trim?.() === ""
    ...
    // Si no especifican módulos, por defecto basic_info
    if (!modulesIn.length) modulesIn = [{ key: "basic_info", fields: null }];
    ...
    for (const m of modulesIn) {
      if (validKeys.has(m.key)) prelimValid.push(m);
      else invalidKeys.push(m.key);
    }
    ...
    if (!filteredModules.length) {
      const out = {
        ok: false,
        error: "no_valid_modules_after_field_validation",
      };
    ...
    return res.status(202).json({
      ...
      warnings: [
        ...(invalidKeys.length
          ? [{ type: "invalid_modules_ignored", keys: invalidKeys }]
          : []),
        ...fieldWarnings,
      ],
      executedBy: {
