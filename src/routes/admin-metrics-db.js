 ...
 *   - top (número): máx de bots en ranking (default 30)
 *   - hours (número): ventana fallback para throughput (default 48)
 *   - days (número): ventana para status succ/fail en días (default 14)
 *   - limit (número): muestras para 'durations' (default 400)
 *   - agg: 'hour' | 'day' | 'month'
 *   - date: YYYY-MM-DD   (si agg=hour)
 *   - month: YYYY-MM     (si agg=day)
 *   - year: YYYY         (si agg=month)
 */
router.get("/metrics-db", adminGate, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // -------- Ranking por bot con promedio de duración --------
      ...
      // -------- Durations (últimos N jobs terminados) --------
      ...
      // -------- Métricas agregadas para las cards --------
      ...
      // -------- Throughput (con series completas) --------
      async function throughputSeries() {
        ...
        // Fallback: últimas N horas completas, hasta la hora actual.
        const q = await client.query(
          `
          WITH bounds AS (
            SELECT
      ...
      // -------- Status succ vs fail (últimos N días) --------
