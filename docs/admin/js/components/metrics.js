// docs/admin/js/components/metrics.js
const MetricsUI = (() => {
  ...
  let thruMode = "hour"; // 'hour' (día), 'day' (mes), 'month' (año)
  ...
  function smallChartPanel(title, subtitle) {
    const el = document.createElement("div");
  ...
  function drawAvgByBot(panelEl, byBot) {
    ...
    // Eje Y 0..20 con grilla 0.5, pero etiquetas cada 2.0s y más padding izq.
    Chart.barChart(cvs, items, {
      height: 420,
      yMin: 0,
      yMax: 13,
      yStep: 0.5,
      yLabelEvery: 1.5, // 0.5 * 4 = 2.0s
      paddingLeft: 52,
    });
  }
  ...
  function controls() {
      ...
      <div class="row" style="gap:12px; align-items:center;">
        <label style="min-width:220px">Gráfica
          <select id="chart-type">
            <option value="avg">Avg Duration por Bot</option>
            <option value="hist">Distribución de Duraciones</option>
            <option value="thru">Throughput</option>
            <option value="status">Estado: success vs fail</option>
          </select>
        </label>
    ...
    sel.addEventListener("change", () => {
      ...
      renderUI(); // re-dibuja con snapshot actual
    });

    // Controles de throughput (solo visibles en esa vista)
    const thruHost = wrap.querySelector("#thru-controls");
    if (currentChart === "thru") {
      const thruHTML = `
        <label>Vista
          <select id="thru-mode">
            <option value="hour">Día (horas)</option>
            <option value="day">Mes (días)</option>
            <option value="month">Año (meses)</option>
          </select>
        </label>
      ...
      modeSel.addEventListener("change", async () => {
        ...
        updateVisibility();
        await loadSnapshotFromDB(); // recarga con nuevos params
      });
  ...
  function renderUI() {
    ...
    // Gráfica principal
    let title = "";
    let subtitle = "Desde BD (snapshot)";
    ...
    // Redibujar al redimensionar (una sola vez por render)
  ...
  async function loadSnapshotFromDB() {
    ...
    try {
      ...
      const mdb = await AdminAPI.metricsDB(thruParams, { signal: ctrl.signal });
      ...
      root.innerHTML = `<div class="panel error">No se pudo cargar métricas (BD): ${
        e?.message || e
      }</div>`;
    } finally {
      currentAbort = null;
  ...
  // ---------- lifecycle ----------
  function start() {
    // reset throughput controls cada vez que entras
    const now2 = new Date();
